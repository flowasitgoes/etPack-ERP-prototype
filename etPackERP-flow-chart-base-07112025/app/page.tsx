      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">工廠ERP系統</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">本月新增 +3</p>
                </CardContent>
              </Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {orders.map((order) => (
                    <div
                      key={order.id}
                      className={`p-4 rounded-lg border-2 cursor-pointer transition-all hover:shadow-md ${getOrderColor(order.color)} ${selectedOrder.id === order.id ? "ring-2 ring-blue-500" : ""}`}
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium">當前課別: {order.currentDepartment}</span>
                        <span className="text-sm text-gray-500">{order.currentDate}</span>
                      </div>
              ...
              <CardContent>
                <div className="flex items-center justify-between mb-6">
                  {departments.map((dept, index) => {
                    ...
                    const isActive = dept.name === selectedOrder.currentDepartment
                    const isCompleted = selectedOrder.progress > index * 25
                ...
                <Card className="mt-6">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      當前課別: {selectedOrder.currentDepartment}
                      <Badge variant="outline">{selectedOrder.currentDate}</Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span>業務課開單完成 (7/14)</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <Clock className="w-4 h-4 text-blue-500" />
                            <span>抽袋課處理中 (7/18)</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <Card key={dept.id}>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {forms[dept.id as keyof typeof forms]?.map((form, index) => (
